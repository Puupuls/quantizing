...
gptq_params = {
    ...
    'disable_exllama': tune.choice([True, False]),
    'model_seqlen': tune.choice([512, 1024, 2048]),
    'group_size': tune.choice([64, 128, 256, 512, -1]),
...
def quantization_process(config):
    ...
    losses = [res[f'loss_{i}'] for i in range(5)]
...
for space in param_spaces:
    ...
    # Run 20 samples of sample space but do not repeat if space is smaller
    analysis = tune.run(
